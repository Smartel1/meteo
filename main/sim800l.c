...
// This function waits for certain response.
...
// If sim800l doesnt respond with new symbols for 3 sec, function returns 0;
...
int waitResponse(char *response) {
    ...
    while (esp_timer_get_time() - start_time < 5000000) {
        ...
        bool ok = strcmp(line_buf, response) == 0;
        if (ok) {
            return 1;
        }
    }
    ESP_LOGE(TAG, "desired response not received");
